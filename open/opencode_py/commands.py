...
def _mcp(ctx: CommandContext, args: str) -> None:
    ...
    if sub in ("remove", "rm", "del", "delete"):
        ...
        if getattr(ctx, "preview_only", False):
            ...
            return
        path = _mm.remove_server(name, ctx.worktree, is_global)
    ...
    if sub in ("test", "check", "ping"):
        ...
        if getattr(ctx, "preview_only", False):
            ...
            return
        ctx.reply("\n".join(
            _mm.test_server(n, s.get("command"), (s.get("args") or []))
            for n, s in servers.items() if isinstance(s, dict) and s.get("command")
        ))
        return
    ctx.reply(
